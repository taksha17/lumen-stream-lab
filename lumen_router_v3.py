"""Learned router v3 — same linear model as v2, weights from route-log training."""
...
WEIGHTS_PATH = ROOT / "data" / "router-v3-weights.json"
...
CLASS_TO_REASON = {
    "fast": "router-v3/fast",
    "balanced_lfm": "router-v3/balanced/general",
    "balanced_domain": "router-v3/balanced/domain",
    "quality": "router-v3/quality",
}
...
def _load_weights() -> dict:
    if not WEIGHTS_PATH.exists():
        raise FileNotFoundError(
            f"missing {WEIGHTS_PATH}; run scripts/seed_route_log.py && scripts/train_router_v3.py"
        )
...
def route_decision_v3(prompt: str, tier_pref: str = "auto") -> dict[str, Any]:
    if tier_pref != "auto":
